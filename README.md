...
## النشر السحابي — معاينة بضغطة واحدة (Deploy a Preview)

[![Deploy to Render](https://render.com/images/deploy-to-render-button.svg)](https://render.com/deploy?repo=https://github.com/rawasi114/rwasisama)

ينشر هذا الزر معاينة تجريبية عبر [Render](https://render.com) باستخدام ملف `render.yaml`
(خدمة API + الواجهة + قاعدة PostgreSQL مُدارة). الخطوات:

1. اضغط الزر أعلاه، وسجّل الدخول إلى Render عبر GitHub.
2. اربط المستودع `rawasi114/rwasisama` ووافق على الـ Blueprint.
3. انتظر اكتمال البناء (~٥–١٠ دقائق). ستحصل على رابطين: `rawasi-web` (الواجهة) و `rawasi-api`.
4. افتح رابط `rawasi-web` وسجّل الدخول بحساب الـ admin أعلاه.

> ⚠️ **معاينة ببيانات وهمية فقط — ليست للإنتاج.** الاستضافة الإنتاجية لبيانات العملاء
> الحكوميين/العسكريين تتطلب قراراً حول سيادة البيانات داخل السعودية (سؤال مفتوح #1 في `PLAN.md`).
> إن اختلف اسم خدمة الـ API عند الإنشاء، حدّث المتغيّر `API_INTERNAL_URL` في خدمة `rawasi-web`.

**المعمارية:** الواجهة تستدعي `/api/*` من نفس المصدر، ويعيد Next توجيهها (proxy) إلى الـ API —
بلا أي إعداد CORS. تم التحقق من هذا المسار محلياً (تسجيل دخول عبر `:3000/api` → API → DB).
