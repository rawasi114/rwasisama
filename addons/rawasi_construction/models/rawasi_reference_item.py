...
REFERENCE_CODE_PATTERN = re.compile(
    # يدعم النمطين:
    #   ① الصيغة الكلاسيكية:  FLR-POR-6060-001  (3-4 أحرف ثم رقم 3 خانات)
    #   ② الصيغة الموحَّدة:    RSC.03.01.01.001  (بادئة + 4 مستويات رقمية)
    r"^(?:[A-Z]{2,4}-[A-Z]{2,4}-[A-Z0-9]{2,8}-\d{3}|[A-Z]{2,4}\.\d{2}\.\d{2}\.\d{2}\.\d{3})$"
)
...
class ReferenceItem(models.Model):
    ...
    # ═══ حقول قاعدة البنود الموحَّدة (نموذج C+B الهجين) ═══

    # ① الوصف الكامل كما ورد من الجهة الحكومية (للأرشيف والمطابقة)
    government_text = fields.Text(
        string="الوصف الكامل من الجهة",
        translate=True,
        help="النص كاملاً كما ورد في جدول كميات الجهة الحكومية. "
             "يُملأ آلياً عند أول مطابقة، ويبقى مرجعاً نصياً للعقود.",
    )

    # ② صياغة عرض السعر الجاهزة (توريد وتركيب) — من B
    quotation_template = fields.Html(
        string="صياغة عرض السعر (توريد وتركيب)",
        translate=True,
        sanitize=True,
        help="صياغة احترافية مختصرة جاهزة للنسخ في عرض السعر — "
             "تشمل التوريد والتركيب والمعالجة.",
    )

    # ③ كلمات المطابقة — تغذية لمنظومة matching_engine
    matching_keywords = fields.Char(
        string="كلمات المطابقة",
        help="كلمات/مرادفات مفصولة بفواصل تعزّز دقّة المطابقة الذكية. "
             "مثال: «بورسلين، porcelain، 60x60، أرضيات».",
    )

    # ④ تصنيف أودو المقترح (مادة/خدمة)
    product_type_odoo = fields.Selection(
        [
            ("product", "مادة"),
            ("service", "خدمة"),
            ("combined", "توريد وخدمة"),
        ],
        string="تصنيف أودو",
        default="product",
        help="يحدّد سلوك البند عند التحويل لمنتج أودو لاحقاً.",
    )

    # ⑤ قاعدة التسعير الداخلية
    pricing_rule = fields.Text(
        string="قاعدة التسعير الداخلية",
        help="إرشادات داخلية للمكتب الفني عن كيف يُسعَّر هذا البند.",
    )

    # ⑥ تنبيه التسعير
    pricing_alert = fields.Text(
        string="تنبيه التسعير",
        help="تحذير يظهر عند تسعير هذا البند (تذبذب الأسواق، تبعات جودة...).",
    )

    # ⑦ نموذج مواصفة فعلية مرجعية
    sample_specification = fields.Text(
        string="نموذج مواصفة مرجعية",
        help="مواصفة فعلية مأخوذة من مشاريع سابقة للاسترشاد.",
    )
